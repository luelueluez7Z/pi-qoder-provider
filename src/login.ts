...
// must use those callbacks directly rather than opening our own UI surface
// (which would steal focus and leave onPrompt unable to receive keystrokes).
...
async function patLogin(
  ...
  if (!pat) throw new Error("No Personal Access Token provided");
...
async function runDeviceFlow(callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
  ...
  // Track consecutive network failures separately from total attempts. Network
  // blips (DNS/IP fallback, transient timeouts) should not abort the whole login
  // when the user has already authorized in the browser — keep polling.
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      // Network-level failure (TypeError "fetch failed" / timeout). The user may
      // have already authorized in the browser, so don't abort — retry polling.
      consecutiveNetworkErrors++;
      if (consecutiveNetworkErrors <= maxConsecutiveNetworkErrors) {
        getProgress(callbacks)?.("Network hiccup, retrying...");
        continue;
      }
      throw e;
    }
