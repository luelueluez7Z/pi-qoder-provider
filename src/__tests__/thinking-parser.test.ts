...
describe("ThinkingTagParser", () => {
  ...
  it("handles  thinking tags", () => {
    const parser = new ThinkingTagParser(output, stream);
    parser.processChunk("Hello  thinkingreasoning response world");
    parser.finalize();
  ...
  it("handles text ending with partial tag prefix (flushes on finalize)", () => {
    const parser = new ThinkingTagParser(output, stream);
  ...
  it("emits text_start/text_delta and thinking_start/thinking_delta/thinking_end events", () => {
    const parser = new ThinkingTagParser(output, stream);
    parser.processChunk("Hi <thinking>deep</thinking>");
    parser.finalize();
  ...
  // Regression: Qoder splits one <thinking> pair across two SSE fields — opener
  // into reasoning_content, closer into content. The closer has no opener in the
  // content stream and must be dropped instead of leaking into visible text.
