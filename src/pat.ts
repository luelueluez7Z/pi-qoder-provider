...
/**
 * @deprecated Legacy encoder that embedded the PAT. Kept for compatibility;
 * new code must use encodeJobRefresh.
 */
export function encodePatRefresh(pat: string, jobRefreshToken: string, userID: string, machineID: string): string {
  void pat; // never persist plaintext PATs in refresh
...
export async function credentialsFromPat(pat: string, mode: string = getQoderMode()): Promise<OAuthCredentials> {
  ...
  if (!jobRefreshToken) {
    throw new Error(
      isQoderCNMode(mode)
        ? "Qoder CN login failed: jobToken exchange returned no refresh_token (jrt). Cannot persist a refreshable session without embedding the PAT."
...
/** Build credentials from a job refresh token response / existing identity. Never accepts/stores a PAT. */
export function credentialsFromJobTokens(
  ...
  if (!identity.userID) throw new Error("Cannot build Qoder credentials without userID");
